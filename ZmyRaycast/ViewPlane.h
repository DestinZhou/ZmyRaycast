...
class ViewPlane {
public:
	int 			hres;   					// horizontal image resolution	水平图形像素
	int 			vres;   					// vertical image resolution	垂直图形像素
	float			s;							// pixel size					像素大小
	int				num_samples;				// number of samples per pixel	每个像素点的采样

	float			gamma;						// gamma correction factor		gamma矫正因子
	float			inv_gamma;					// the inverse of the gamma correction factor  1/gamma
	bool			show_out_of_gamut;			// display red if RGBColor out of gamut			是否超过颜色值范围时，显示红色
